...
foreach ($root in $roots) {
    if ((Test-Path (Join-Path $root ".git")) -and ((Split-Path $root -Leaf) -eq "CKS")) { $localRepo = Get-Item $root; break }
    $candidate = Join-Path $root "CKS"
    if (Test-Path (Join-Path $candidate ".git")) { $localRepo = Get-Item $candidate; break }
}

if (-not $localRepo) { throw "Local CKS repository not found" }

Write-Host "Local CKS found: $($localRepo.FullName)"
Write-Host "Execution locked: $($localRepo.FullName)"
...
$apiPath = "repos/$RemoteRepo/contents/$ToolPath"
...
& $download -RepoPath $localRepo.FullName
